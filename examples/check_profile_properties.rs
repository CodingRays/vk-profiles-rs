...
fn main() {
    ...
    let profiles = unsafe {
        vk_profiles
            .get_profiles()
            .expect("Failed to enumerate supported profiles")
    };
...
fn print_profile_property_support(vk_profiles: &VulkanProfiles, profile: &vp::ProfileProperties) {
        ...
        get_max_descriptor_set_update_after_bind_input_attachments(&vk_profiles, &profile).unwrap()
    {
        ...
        println!(
            "Profile {:?} does not include VK_EXT_descriptor_indexing or vulkan 1.2",
            profile
        );
    }
}

fn get_max_descriptor_set_update_after_bind_input_attachments(
    ...
    if property_types.contains(&vk::StructureType::PHYSICAL_DEVICE_DESCRIPTOR_INDEXING_PROPERTIES) {
        ...
        let mut properties = vk::PhysicalDeviceDescriptorIndexingProperties::default();
        ...
        let mut properties = vk::PhysicalDeviceVulkan12Properties::default();
